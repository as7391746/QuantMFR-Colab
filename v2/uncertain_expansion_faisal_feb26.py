...
def uncertain_expansion(control_variables, state_variables, shock_variables, variables, variables_tp1,
    ...
    # v2: if the caller supplies no starting vector, derive one from the
    # model itself (see auto_guess.py in this sandbox)
    if initial_guess is None:
        from auto_guess import derive_initial_guess
        initial_guess = derive_initial_guess(
            ss_variables, control_variables, state_variables,
            shock_variables, output_constraint, capital_growth,
            state_equations, static_constraints, parameter_names, args)
