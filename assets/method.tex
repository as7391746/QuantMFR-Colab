...
\node[stage, minimum width=95pt] (model) at (0,0)
  {\textbf{MODEL}\\[1pt]
   ...
   \scriptsize $\nu_1,\nu_2,\mu_2,\sigma_\cdot$ (appendix)};

\node[order, right=26pt of model] (o0)
  {\textbf{order 0}\\[1pt]\scriptsize deterministic\\
   \scriptsize steady state $(D_2^0, v^0)$};
\node[order, right=18pt of o0] (o1)
  {\textbf{order 1}\\[1pt]\scriptsize linear responses $(\upsilon_1, d_1)$\\
   \scriptsize $\mu^0=(1{-}\gamma_o)\,\Sigma_w$};
\node[order, right=18pt of o1] (o2)
  {\textbf{order 2}\\[1pt]\scriptsize quadratic corrections $(v^2, d^2)$\\
   \scriptsize expectations under $N^0$};
...
\end{scope}
\node[lbl, anchor=south] at (solvebox.north)
  {SOLVE — \texttt{uncertain\_expansion}, the book's expansion code (``approach one'')};
...
% ---------- cell mapping ----------
\node[cell, anchor=north] at ($(model.south)+(0,-4pt)$)   {cell 2};
\node[cell, anchor=north] at ($(solvebox.south)+(0,-4pt)$){cell 3};
\node[cell, anchor=north] at ($(price.south)+(0,-4pt)$)   {cell 3};
\node[cell, anchor=north] at ($(figs.south)+(0,-4pt)$)    {cells 4--6};
\end{tikzpicture}
\end{document}
